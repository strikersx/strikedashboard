...
export default function TrialsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="head text-sm font-semibold text-muted-strong uppercase tracking-wide mb-3">Alunos com aula experimental (<span className="num">{students.length}</span>)</h2>
        <div className="space-y-1.5">
          {students.map((c) => (
            <div key={c.id} className={`bg-surface rounded-lg p-3 flex items-center justify-between gap-3 border-l-4 ${c.attended ? "border-accent" : "border-border-subtle"}`}>
      ...
      {classes.length > 0 && (
        <div>
          <h2 className="head text-sm font-semibold text-muted-strong uppercase tracking-wide mb-3">Aulas com inscrições</h2>
